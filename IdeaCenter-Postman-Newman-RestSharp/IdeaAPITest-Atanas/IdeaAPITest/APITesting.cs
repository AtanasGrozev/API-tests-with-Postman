...
namespace IdeaAPITest
{
    public class Tests
    {
        ...
        private const string EMAIL = "atanas@test.bg";
        ...
        private string GetJwtToken(string email, string password) // създава токена
        {
        ...
        [Test, Order(1)]
        public void CreateNewIdea_WithCorrectData_shoudSucceed()
        {
        ...
        [Test, Order(2)]
        public void GetAllIdeas_ShoudReturnNonEmptyArray()
        {
            var requestData = new IdeaDTO()
            {
                Title = "TestTitle",
        ...
        [Test, Order(5)]
        public void CreateNewIdea_WithoutCorrectData_shoudFail()
        {
            var requestData = new IdeaDTO()
            {
                Title = "TestTitle",
        ...
        public void EditWithWrongID_ShouldFail()
        {
            //Assume.That(!string.IsNullOrEmpty(lastCreatedIdeaId), "CreateIdea test must run first and succeed.");
            ...
            request.AddQueryParameter("ideaId", "11223344");
            request.AddJsonBody(editRequest);
            var response = this.client.Execute(request);
        ...
        public void DeleteIdeaWithWrongId_ShouldFail()
        {
            ...
            request.AddQueryParameter("ideaId", "2233545");
            var response = this.client.Execute(request);
